...
' 12.If necessary rename module 'Build1' to FuncoesVersionador. Menu File-->Save vbaDeveloper.xlam
...
Public Sub ImportarModulos()
    ...
    proj_name = "ControlDocsProject"

    Dim vbaProject As Object
    Set vbaProject = Application.VBE.VBProjects(proj_name)
    FuncoesVersionador.importVbaCode vbaProject
...
Public Sub ExportarModulos()
    ...
    Dim vbaProject As Object
    Set vbaProject = Application.VBE.VBProjects(proj_name)
    FuncoesVersionador.exportVbaCode vbaProject
...
Public Sub importVbaCode(vbaProject As VBProject, Optional includeClassFiles As Boolean = False)
    ...
    Next
    'Then import them
    Debug.Print "Invoking 'FuncoesVersionador.importComponents'with Application.Ontime with delay " & IMPORT_DELAY
    ' to prevent duplicate modules, like MyClass1 etc.
    Application.OnTime Now() + TimeValue(IMPORT_DELAY), "'FuncoesVersionador.importComponents'"
...
Public Function IdComputador() As String
...
Dim Uuid As Variant
    ...
    'Executar o comando para obter o UUID do sistema
    Uuid = objShell.Exec("wmic csproduct get UUID").StdOut.ReadAll
    
    Uuid = VBA.Split(Uuid, vbCrLf)
    Uuid = VBA.Left(Uuid(1), 36)

    IdComputador = Uuid
    
End Function

Function VerificarProjeto() As Boolean
...
Dim vbProj As VBIDE.VBProject
    ...
    If Not vbProj Is Nothing Then VerificarProjeto = True
