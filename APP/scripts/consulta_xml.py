...
def formatar_data_excel(data):
    try:
        ...
        print(f" [01]-Erro ao converter data: {data} -> {e}")
        return ""
...
def main():
    ...
    print(f" [00]-Executando consulta_sefaz com planilha: {caminho_planilha}")
    ...
    try:
        ...
        print(f" [01]-Erro ao ler a planilha: {e}")
        sys.exit(1)
    ...
    for idx, row in df.iterrows():
        ...
        if not inscricao:
            print(f" [01]-Linha {idx + 2}: Inscrição Municipal ausente. Pulando...")
            continue

        print(f" [00]-Consultando Inscrição Municipal: {inscricao}")
    ...
    print(" [00]-Processamento concluído para todas as empresas.")
