...
# ==============================================================================
def main():
    parser = argparse.ArgumentParser(description="Processador para extratos do banco BNB com múltiplas contas.")
    ...
    if not indices_inicio_contas:
        print("Nenhuma seção de Agência/Conta foi encontrada no documento.")
        return

    print(f"Mapeamento concluído. Encontrado(s) {len(indices_inicio_contas)} extrato(s) de conta no arquivo.\n")
    
    mes_ano_global = extrair_mes_ano(linhas_brutas)
    if not mes_ano_global:
        print("ERRO: Não foi possível encontrar o Mês/Ano de referência no extrato.")
        return
    ...
    for i, start_index in enumerate(indices_inicio_contas):
        ...
        print(f"--- Processando Extrato #{i+1}: Agência {agencia} / Conta {conta} ---")

        bloco_lancamentos = extrair_bloco_de_lancamentos_bnb(bloco_conta_atual)
        if not bloco_lancamentos:
            print("   -> Nenhum bloco de lançamentos encontrado para esta conta.\n")
            continue
        ...
        if todos_lancamentos:
            ...
            print(f"   -> Resultado salvo em: '{caminho_arquivo_saida}'\n")
        else:
            print("   -> Nenhum lançamento válido foi extraído para esta conta.\n")
